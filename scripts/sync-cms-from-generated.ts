...
const SERVICES: Array<{
  ...
  },
  {
    ...
    durationMin: 60,
    bookable: true,
    images: ["/media/home/endospheres.jpg"],
  },
  {
    slug: "endospheres",
...
async function syncServices(
  ...
  for (const definition of SERVICES) {
    ...
    for (const [locale, content] of Object.entries(
      ...
      const data = {
        h1: content.title,
        shortDesc: excerpt(content.body),
        whatItIs: content.body,
        suitableFor: [],
        benefits: [],
        processSteps: [],
        safety: "",
...
async function main() {
  ...
  console.log(
    `${force ? "Force-refreshed" : "Created missing"}: ${pageCount} pages, ${serviceCount} services, ${technologyCount} technologies, ${productCount} products.`,
  );
}
