...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: localeAlternates(PUBLIC_PATHS.services, locale),
  };
}
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-[clamp(16px,1.8vw,26px)]">
          {services.map((service, index) => {
            ...
            return (
              <article
                key={service.id}
                className="group flex min-h-full flex-col overflow-hidden rounded-[var(--radius)] border border-line-card bg-card transition-all duration-300 hover:-translate-y-[6px] hover:border-line-card-hover hover:shadow-card motion-reduce:transform-none"
              >
                <Link
                  href={service.publicPath}
                  className="relative block h-[248px] overflow-hidden bg-page"
                >
